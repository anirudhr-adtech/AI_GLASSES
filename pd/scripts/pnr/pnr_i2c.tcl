###############################################################################
# OpenROAD PnR Script — I2C Master Subsystem
# Usage:  openroad -exit scripts/pnr/pnr_i2c.tcl
...
#   - 1,989 cells, ~22K um^2 stdcell area
#   - Die 200x200, core 180x180 -> ~50% utilization
...
set NETLIST    results/synth/i2c_master_netlist.v
set SDC        constraints/i2c_master_pnr.sdc
...
# ==============================================================
puts "\n===== Step 2: Floorplan ====="
# Stdcell area: ~22K um^2 (1,989 cells from synthesis)
# Die 280x280, core 260x260 = 67,600 um^2 -> ~33% util
initialize_floorplan \
    -die_area "0 0 280 280" \
    -core_area "10 10 270 270" \
    -site unithd
...
# Report placement
report_design_area > ${RPT_DIR}/i2c_placement_area.rpt
puts "\n--- Placement Summary ---"
report_design_area
...
# CTS report
report_clock_skew > ${RPT_DIR}/i2c_cts_skew.rpt
puts "\n--- Clock Skew Report ---"
report_clock_skew
...
# Global routing
global_route -guide_file ${OUT_DIR}/i2c_route.guide \
    -congestion_iterations 50 \
    -allow_congestion

# Detailed routing
detailed_route \
    -output_drc ${RPT_DIR}/i2c_route_drc.rpt
...
report_checks -path_delay max -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/i2c_postroute_setup.rpt
report_checks -path_delay min -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/i2c_postroute_hold.rpt
report_tns > ${RPT_DIR}/i2c_postroute_tns.rpt
report_wns > ${RPT_DIR}/i2c_postroute_wns.rpt

# Design area
report_design_area > ${RPT_DIR}/i2c_postroute_area.rpt
...
# 9. Write outputs
# ==============================================================
write_def ${OUT_DIR}/i2c_master_routed.def
write_verilog ${OUT_DIR}/i2c_master_pnr.v

puts "\n=========================================="
puts " I2C Master PnR Complete"
puts " DEF:     ${OUT_DIR}/i2c_master_routed.def"
puts " Netlist: ${OUT_DIR}/i2c_master_pnr.v"
puts " Reports: ${RPT_DIR}/i2c_*.rpt"
puts "=========================================="
