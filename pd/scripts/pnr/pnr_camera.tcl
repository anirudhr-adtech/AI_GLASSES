###############################################################################
# OpenROAD PnR Script — Camera Subsystem
# Usage:  openroad -exit scripts/pnr/pnr_camera.tcl
...
#   - 119K cells, 1.77 mm² stdcell area
#   - Die 2200x2200, core 2160x2160 (~38% util, room for buffers)
#   - No external macros (line buffers synthesized to FFs)
#   - DVP pclk domain false-pathed in SDC
#   - repair_design -max_wire_length 1000 (large die)
#   - global_placement -density 0.5 (more room for routing)
###############################################################################
...
set NETLIST    results/synth/cam_subsys_top_netlist.v
set SDC        constraints/cam_subsys_top_pnr.sdc
...
# ==============================================================
puts "\n===== Step 2: Floorplan ====="
# Stdcell area: ~1,770K um² (119K cells from synthesis)
# Die 2200x2200, core 2160x2160 = 4,665K um² -> ~38% util
...
# Vertical stripes (met4) — wider pitch for large die
add_pdn_stripe -grid core_grid -layer met4 -width 1.6 -pitch 80.0 -offset 13.0

# Horizontal stripes (met5)
add_pdn_stripe -grid core_grid -layer met5 -width 1.6 -pitch 80.0 -offset 13.0
...
# Global placement — density 0.5 for room on large die
...
# Report placement
report_design_area > ${RPT_DIR}/camera_placement_area.rpt
puts "\n--- Placement Summary ---"
report_design_area
...
clock_tree_synthesis \
    ...
    -sink_clustering_size 40 \
    -sink_clustering_max_diameter 150
...
# CTS report
report_clock_skew > ${RPT_DIR}/camera_cts_skew.rpt
puts "\n--- Clock Skew Report ---"
report_clock_skew
...
# Repair design — max wire length 1000 for large 2200um die
...
# Global routing
global_route -guide_file ${OUT_DIR}/camera_route.guide \
    -congestion_iterations 50 \
    -allow_congestion

# Detailed routing
detailed_route \
    -output_drc ${RPT_DIR}/camera_route_drc.rpt
...
report_checks -path_delay max -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/camera_postroute_setup.rpt
report_checks -path_delay min -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/camera_postroute_hold.rpt
report_tns > ${RPT_DIR}/camera_postroute_tns.rpt
report_wns > ${RPT_DIR}/camera_postroute_wns.rpt

# Design area
report_design_area > ${RPT_DIR}/camera_postroute_area.rpt
...
# 9. Write outputs
# ==============================================================
write_def ${OUT_DIR}/cam_subsys_top_routed.def
write_verilog ${OUT_DIR}/cam_subsys_top_pnr.v

puts "\n=========================================="
puts " Camera PnR Complete"
puts " DEF:     ${OUT_DIR}/cam_subsys_top_routed.def"
puts " Netlist: ${OUT_DIR}/cam_subsys_top_pnr.v"
puts " Reports: ${RPT_DIR}/camera_*.rpt"
puts "=========================================="
