###############################################################################
# OpenROAD PnR Script — SPI Master Subsystem
# Usage:  openroad -exit scripts/pnr/pnr_spi.tcl
...
#   - 1,342 cells, ~16K um^2 stdcell area
#   - Die 180x180, core 160x160 -> ~50% utilization
...
set NETLIST    results/synth/spi_master_netlist.v
set SDC        constraints/spi_master_pnr.sdc
...
# ==============================================================
puts "\n===== Step 2: Floorplan ====="
# Stdcell area: ~16K um^2 (1,342 cells from synthesis)
# Die 180x180, core 160x160 = 25,600 um^2 -> ~50% util
...
# Report placement
report_design_area > ${RPT_DIR}/spi_placement_area.rpt
puts "\n--- Placement Summary ---"
report_design_area
...
# CTS report
report_clock_skew > ${RPT_DIR}/spi_cts_skew.rpt
puts "\n--- Clock Skew Report ---"
report_clock_skew
...
# Repair design
repair_design -max_wire_length 250
...
# Global routing
global_route -guide_file ${OUT_DIR}/spi_route.guide \
    -congestion_iterations 50 \
    -allow_congestion

# Detailed routing
detailed_route \
    -output_drc ${RPT_DIR}/spi_route_drc.rpt
...
report_checks -path_delay max -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/spi_postroute_setup.rpt
report_checks -path_delay min -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/spi_postroute_hold.rpt
report_tns > ${RPT_DIR}/spi_postroute_tns.rpt
report_wns > ${RPT_DIR}/spi_postroute_wns.rpt

# Design area
report_design_area > ${RPT_DIR}/spi_postroute_area.rpt
...
# 9. Write outputs
# ==============================================================
write_def ${OUT_DIR}/spi_master_routed.def
write_verilog ${OUT_DIR}/spi_master_pnr.v

puts "\n=========================================="
puts " SPI Master PnR Complete"
puts " DEF:     ${OUT_DIR}/spi_master_routed.def"
puts " Netlist: ${OUT_DIR}/spi_master_pnr.v"
puts " Reports: ${RPT_DIR}/spi_*.rpt"
puts "=========================================="
