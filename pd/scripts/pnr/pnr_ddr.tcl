###############################################################################
# OpenROAD PnR Script — DDR Wrapper Subsystem
# Usage:  openroad -exit scripts/pnr/pnr_ddr.tcl
...
#   - 4,535 cells, ~37K um^2 stdcell area
#   - Die 250x250, core 230x230 -> ~50% utilization
...
set NETLIST    results/synth/ddr_wrapper_netlist.v
set SDC        constraints/ddr_wrapper_pnr.sdc
...
# ==============================================================
puts "\n===== Step 2: Floorplan ====="
# Stdcell area: ~37K um^2 (4,535 cells from synthesis)
# Die 350x350, core 330x330 = 108,900 um^2 -> ~34% util
initialize_floorplan \
    -die_area "0 0 350 350" \
    -core_area "10 10 340 340" \
    -site unithd
...
# Report placement
report_design_area > ${RPT_DIR}/ddr_placement_area.rpt
puts "\n--- Placement Summary ---"
report_design_area
...
# CTS report
report_clock_skew > ${RPT_DIR}/ddr_cts_skew.rpt
puts "\n--- Clock Skew Report ---"
report_clock_skew
...
repair_design -max_wire_length 350
...
# Global routing
global_route -guide_file ${OUT_DIR}/ddr_route.guide \
    -congestion_iterations 50 \
    -allow_congestion

# Detailed routing
detailed_route \
    -output_drc ${RPT_DIR}/ddr_route_drc.rpt
...
report_checks -path_delay max -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/ddr_postroute_setup.rpt
report_checks -path_delay min -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/ddr_postroute_hold.rpt
report_tns > ${RPT_DIR}/ddr_postroute_tns.rpt
report_wns > ${RPT_DIR}/ddr_postroute_wns.rpt

# Design area
report_design_area > ${RPT_DIR}/ddr_postroute_area.rpt
...
# 9. Write outputs
# ==============================================================
write_def ${OUT_DIR}/ddr_wrapper_routed.def
write_verilog ${OUT_DIR}/ddr_wrapper_pnr.v

puts "\n=========================================="
puts " DDR Wrapper PnR Complete"
puts " DEF:     ${OUT_DIR}/ddr_wrapper_routed.def"
puts " Netlist: ${OUT_DIR}/ddr_wrapper_pnr.v"
puts " Reports: ${RPT_DIR}/ddr_*.rpt"
puts "=========================================="
