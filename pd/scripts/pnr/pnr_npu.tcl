###############################################################################
# OpenROAD PnR Script — NPU Subsystem
# Usage:  openroad -exit scripts/pnr/pnr_npu.tcl
# Run from: pd/ directory
#
# Flow: Read netlist → Floorplan → Power grid → Placement → CTS → Route
#
# Key design decisions:
#   - 100 MHz target (10ns) for SKY130 — 200 MHz is for SCL 180nm sign-off only
#   - Die sized for ~50% stdcell utilization (leaves room for CTS buffers)
#   - SRAM macro (400x300) placed at top of die
#   - SRAM macro set as dont_touch after CTS to prevent resizer buffer explosion
#   - Relaxed repair_design constraints for reasonable buffer count
###############################################################################

set TOP        npu_top
set LIB_DIR    $::env(PDK_ROOT)/sky130A/libs.ref/sky130_fd_sc_hd/lib
set LEF_DIR    $::env(PDK_ROOT)/sky130A/libs.ref/sky130_fd_sc_hd/lef
...
set LIB_SRAM   lib/npu_act_buf.lib
set NETLIST    results/synth/npu_top_netlist.v
set SDC        constraints/npu_top_pnr.sdc
...
# ==============================================================
puts "\n===== Step 2: Floorplan ====="
# Stdcell area: ~64K µm² (7.3K cells from synthesis)
# SRAM macro: 400x300 = 120K µm²
# Die 700x600, core 660x560 = 369K µm² → ~50% util
initialize_floorplan \
    -die_area "0 0 700 600" \
    -core_area "20 20 680 580" \
    -site unithd
...
# Place SRAM macro at top-center
place_macro -macro_name u_act_buf -location {150 260} -orientation R0
...
# Report placement
report_design_area > ${RPT_DIR}/npu_placement_area.rpt
puts "\n--- Placement Summary ---"
report_design_area
...
# ==============================================================
puts "\n===== Step 5: CTS ====="
# Wire RC from tech LEF layers (correct units from PDK)
...
# Run CTS — larger clusters to keep buffer count reasonable
# Note: dont_touch on SRAM is set AFTER CTS (CTS needs to connect clock pin)
...
# CTS report
report_clock_skew > ${RPT_DIR}/npu_cts_skew.rpt
puts "\n--- Clock Skew Report ---"
report_clock_skew
...
# NOW set dont_touch on SRAM to prevent resizer from buffering its nets
set_dont_touch [get_cells u_act_buf]

# Repair design — relaxed max wire length to prevent buffer explosion
# 700um die → max_wire_length ~half die diagonal is safe
...
# Global routing
global_route -guide_file ${OUT_DIR}/npu_route.guide \
    -congestion_iterations 50 \
    -allow_congestion

# Detailed routing
detailed_route \
    -output_drc ${RPT_DIR}/npu_route_drc.rpt
...
report_checks -path_delay max -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/npu_postroute_setup.rpt
report_checks -path_delay min -sort_by_slack -group_count 10 \
    > ${RPT_DIR}/npu_postroute_hold.rpt
report_tns > ${RPT_DIR}/npu_postroute_tns.rpt
report_wns > ${RPT_DIR}/npu_postroute_wns.rpt

# Design area
report_design_area > ${RPT_DIR}/npu_postroute_area.rpt
...
# 9. Write outputs
# ==============================================================
write_def ${OUT_DIR}/npu_top_routed.def
write_verilog ${OUT_DIR}/npu_top_pnr.v

puts "\n=========================================="
puts " NPU PnR Complete"
puts " DEF:     ${OUT_DIR}/npu_top_routed.def"
puts " Netlist: ${OUT_DIR}/npu_top_pnr.v"
puts " Reports: ${RPT_DIR}/npu_*.rpt"
puts "=========================================="
