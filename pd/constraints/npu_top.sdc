###############################################################################
# SDC Constraints — NPU Subsystem (npu_top)
# Target: 200 MHz (5.0 ns period) on SKY130
# Note: 200 MHz may be aggressive for SKY130; accept 150 MHz if needed.
#       200 MHz target is for SCL 180nm commercial flow.
###############################################################################

# ----------------------------------------------------------------------
# Clock definition
# ----------------------------------------------------------------------
create_clock -name npu_clk -period 5.0 [get_ports clk]

# Clock uncertainty (jitter + skew budget)
set_clock_uncertainty 0.25 [get_clocks npu_clk]

# Clock transition constraint
set_clock_transition 0.15 [get_clocks npu_clk]
...
set_input_delay  -clock npu_clk -max 2.0 $axilite_inputs
set_input_delay  -clock npu_clk -min 0.5 $axilite_inputs
...
set_output_delay -clock npu_clk -max 2.0 $axilite_outputs
set_output_delay -clock npu_clk -min 0.5 $axilite_outputs
...
set_input_delay  -clock npu_clk -max 2.0 $axi_dma_inputs
set_input_delay  -clock npu_clk -min 0.5 $axi_dma_inputs
...
set_output_delay -clock npu_clk -max 2.0 $axi_dma_outputs
set_output_delay -clock npu_clk -min 0.5 $axi_dma_outputs

# ----------------------------------------------------------------------
# Interrupt output
# ----------------------------------------------------------------------
set_output_delay -clock npu_clk -max 2.0 [get_ports irq_npu_done]
set_output_delay -clock npu_clk -min 0.5 [get_ports irq_npu_done]

# ----------------------------------------------------------------------
# Reset — async, false-path (reset tree buffered during CTS/PnR)
# ----------------------------------------------------------------------
set_input_delay -clock npu_clk -max 2.0 [get_ports rst_n]
