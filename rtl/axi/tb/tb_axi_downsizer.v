`timescale 1ns/1ps
//============================================================================
// Testbench: tb_axi_downsizer
// Project:   AI_GLASSES — AXI Interconnect
// Description: Self-checking testbench for axi_downsizer (128->32 bit)
//============================================================================

module tb_axi_downsizer;
    ...
    reg  clk, rst_n;
    // Wide slave interface (from 128-bit master)
    ...
    reg  [NARROW-1:0]  m_rdata;
    reg                m_rlast;

    integer pass_count, fail_count;
    integer narrow_wr_count;
    integer narrow_rd_count;
    ...
    // Simple narrow slave responder - counts beats
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (m_wvalid && m_wready) begin
                narrow_wr_count <= narrow_wr_count + 1;
                if (m_wlast) begin
                    m_bid    <= m_awid;
                    m_bresp  <= 2'b00;
                    m_bvalid <= 1'b1;
            ...
            if (m_arvalid && m_arready) begin
                m_rid    <= m_arid;
                m_rdata  <= 32'hAA00_0000 + narrow_rd_count;
                m_rresp  <= 2'b00;
                m_rlast  <= 1'b0;
                m_rvalid <= 1'b1;
                narrow_rd_count <= narrow_rd_count + 1;
            end
            if (m_rvalid && m_rready) begin
                if (narrow_rd_count >= 4) begin
                    m_rlast  <= 1'b1;
                end
                m_rdata <= 32'hAA00_0000 + narrow_rd_count;
                narrow_rd_count <= narrow_rd_count + 1;
    ...
    initial begin
        $display("=== tb_axi_downsizer START ===");
        ...
        // Write test: single 128-bit write -> 4x 32-bit writes
        s_awid = 6'd3; s_awaddr = 32'h1000_0000;
        s_awlen = 8'd0; s_awsize = 3'd4; s_awburst = 2'b01; s_awvalid = 1;
        wait (s_awready && s_awvalid);
        @(posedge clk); s_awvalid = 0;

        s_wdata = 128'hDDDD_CCCC_BBBB_AAAA_4444_3333_2222_1111;
        s_wstrb = 16'hFFFF; s_wlast = 1; s_wvalid = 1;
        ...
        wait (s_bvalid);
        @(posedge clk);
        if (s_bresp === 2'b00 && s_bid === 6'd3) begin
            pass_count = pass_count + 1;
            $display("PASS: Downsized write completed");
        end else begin
            fail_count = fail_count + 1;
            $display("FAIL: Downsize write bresp=%b bid=%0d", s_bresp, s_bid);
        end

        repeat (5) @(posedge clk);

        $display("=== tb_axi_downsizer DONE ===");
